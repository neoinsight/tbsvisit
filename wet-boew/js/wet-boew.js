...
$document.on( "redir." + actionEvent, selector, function( event, data ) {
	...
	// Do the action upon form submission
...
$document.on( "addClass." + actionEvent, selector, function( event, data ) {
	if ( !data.live ) {
		...
		$( event.target ).trigger( "addClass." + submitEvent, data );
	}
} );
...
$document.on( "removeClass." + actionEvent, selector, function( event, data ) {
	if ( !data.live ) {
		...
		$( event.target ).trigger( "removeClass." + submitEvent, data );
	}
} );
...
$document.on( "tblfilter." + actionEvent, selector, function( event, data ) {
	 ...
	 *
	tblfilterEvent = {
		source:		// [required] Id of the data table where the filtering would be applied to
	 ...
	 */
	// Note: There is no action on submit because this table filtering mode is only supported in live mode.
	if ( event.namespace === actionEvent ) {
...
$document.on( "select." + createCtrlEvent, selector, function( event, data ) {
	 ...
	 *
	{
		...
		defaultselectedlabel: "", // (Optional) specified the empty label
...
function buildDataAttribute( data ) {
	var out = "",
		dataFieldflow = {};
...
$document.on( "checkbox." + createCtrlEvent, selector, function( event, data ) {
	...
	$( this ).trigger( "CheckboxRadio", data );

} );

function buildCheckboxRadio( data, fieldName, inputType, isInline, isReq ) {
	var label = data.label,
		fieldID = wb.getId(),
...
$document.on( "radio." + createCtrlEvent, selector, function( event, data ) {
	data.typeRadCheck = "radio";
	$( this ).trigger( "CheckboxRadio", data );

} );

$document.on( "CheckboxRadio." + createCtrlEvent, selector, function( event, data ) {
